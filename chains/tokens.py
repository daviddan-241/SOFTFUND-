"""
Token definitions, parallel balance checks, and sweep logic.
"""
...
# ── Parallel balance fetch (was sequential → 60s+, now ~3s) ──────
def get_token_balances(w3: Web3, address: str, token_map: dict, timeout: float = 12.0) -> dict:
    """
    Return {symbol: human_balance} for all tokens with balance > 0.
    Runs all RPC calls in parallel — typically 3-5 seconds instead of 60+.
    """
    addr = Web3.to_checksum_address(address)
    out  = {}

    def _check(symbol: str, token_addr: str):
        try:
    ...
    with ThreadPoolExecutor(max_workers=12) as executor:
        futures = {executor.submit(_check, sym, taddr): sym for sym, taddr in token_map.items()}
        for fut in as_completed(futures, timeout=timeout):
            try:
                symbol, amount = fut.result(timeout=3)
                if amount > 0:
                    out[symbol] = amount
            except Exception:
                pass
