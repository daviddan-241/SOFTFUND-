...
from chains.tokens import ETH_TOKENS, get_token_balances
from chains.sweep_engine import smart_sweep_evm, ETH_RPCS
...
def _make_w3():
    return Web3(Web3.HTTPProvider(ETH_RPC_URL, request_kwargs={"timeout": 15}))
...
def get_balance(address: str) -> float:
    for rpc in [ETH_RPC_URL] + ETH_RPCS:
        try:
...
def get_all_balances(address: str) -> dict:
    ...
    try:
        ...
        pass
    w3 = _make_w3()
    balances.update(get_token_balances(w3, address, ETH_TOKENS))
    return balances
...
def forward(secret: str, destination: str, user_id: int) -> list:
    try:
        ...
        return [{"chain": "ethereum", "asset": "ETH", "status": "error",
    ...
    results = smart_sweep_evm(
        ...
        token_map=ETH_TOKENS, rpc_pool=ETH_RPCS, config_rpc=ETH_RPC_URL,
    )
    return [{"chain": "ethereum", **r} for r in results]
